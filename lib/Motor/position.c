...
void 运行前复位(void)
{
    MoveToPosition(未加工到右限位距离, 0, 0, 左右速度);//临时更改-100！！！
    当前x坐标 = 0;
    等待处位置 = 未加工到加工距离 - 3200;
    H170 = E80 + F20 + 夹取位置;
...
void 移动到物料位置(void)
{
    ...
    if(自动端面检测 == 1)
    {
        MoveToPosition(等待处位置, 两端上下距离-30, 0, 左右速度);
        实时测量夹取位置 = 1;
        GoToPage(22); // 检测结束

        motor_go(X_MOTOR, GO_RIGHT, E80, 100);
        实时测量夹取位置 = 0;
        当前x坐标 = 当前x坐标-当前脉冲数;
        给迪文上传数据(Addr当前X位置, 当前x坐标);
        GoToPage(23); // 工件夹取中

        motor_go(X_MOTOR,GO_RIGHT,夹取位置+300,150);
        当前x坐标 = 当前x坐标-当前脉冲数;
        给迪文上传数据(Addr当前X位置, 当前x坐标);
        GoToPage(24); // 工件夹取中
        GoToPage(25);
        MoveToPosition(当前x坐标, 两端上下距离, 0, 左右速度);
        夹取(右);
        GoToPage(26);
        GoToPage(27);
        MoveToPosition(当前x坐标, 0, 0, 左右速度);
        GoToPage(28);
    ...
    else
    {
        ...
        GoToPage(25);
        MoveToPosition(0, 两端上下距离, 0, 左右速度);
        夹取(右);
        GoToPage(26);
        给迪文上传数据(Addr当前脉冲数, 0);
        DelayMs(100);
...
void 调试(void)
{
    ...
    while (1)
    {
        ...
        if (开始调试) // 前往第二个调试点
        {
            ...
            MoveToPosition(夹取位置+500, 两端上下距离-30, 0, 左右速度);
            实时测量夹取位置 = 1;
